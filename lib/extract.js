...
async function fetchHtml(url) {
  ...
  try {
    ...
    clearTimeout(timer);
    // Whatever path exits — a non-2xx, a non-HTML body, a refused redirect —
    // nothing of the request may linger: an unread body would otherwise hold
    // its socket and buffers until GC. No-op once the body is consumed.
    controller.abort();
  }
}
...
// Readability's content parsed once, header chrome removed. Both the
// plain-text and the block views read from this document (neither mutates
// it), so the content is not parsed into a second and third DOM.
export function contentDocument(contentHtml) {
...
export function paragraphsOf(content, fallbackText) {
  if (content) {
    try {
      const nodes = [...content.querySelectorAll('p, li, h2, h3, blockquote')];
...
export function absHttp(href, baseUrl) {
  if (!href) return null;
  try {
...
export function blocksOf(content, baseUrl) {
...
// Extractions in flight at once. Each one parses up to MAX_BODY_BYTES of
// HTML into a DOM — tens of MB while it lives — so a burst of preview opens
// waits its turn here instead of multiplying that peak.
...
async function withSlot(fn) {
  if (inFlight >= MAX_CONCURRENT) {
    await new Promise((resolve) => waiting.push(resolve)); // slot handed over
  } else {
    inFlight += 1;
  }
  try {
    ...
    if (next) next(); // pass the slot straight on, never over-admit
    else inFlight -= 1;
  }
}

export async function extractArticle(rawUrl) {
  assertAllowedUrl(rawUrl, 'requested url'); // refuse before queueing
  return withSlot(() => extract(rawUrl));
}
...
async function extract(rawUrl) {
  ...
  const content = contentDocument(parsed.content);
  let text = paragraphsOf(content, parsed.textContent);
  ...
  return {
    ...
    text,
    blocks: blocksOf(content, finalUrl),
