...
const CHROME_SELECTOR = 'nav, aside, figure figcaption ~ *, [class*="breadcrumb" i], [id*="breadcrumb" i], [class*="share" i], [class*="social" i], [class*="related" i], [class*="newsletter" i], [class*="advertis" i]';
...
function paragraphsOf(contentHtml, fallbackText) {
  if (contentHtml) {
    try {
      ...
      // Drop leading header debris: datelines and short fragment lines
      // (breadcrumbs like "Live", "Middle East") before the first real
      // paragraph; keep everything after prose starts.
      let started = false;
      const paragraphs = [];
      for (const text of texts) {
        ...
        if (!started) {
          ...
          if (DATELINE_RE.test(text) || (words < 5 && !/[.!?…"”]$/.test(text))) continue;
          started = true;
        }
