...
// Feeds emit absolute AND protocol-relative image URLs; normalize both,
...
function mediaUrl(entries) {
  ...
  for (const entry of entries) {
    ...
    const url = attrs && imageUrl(attrs.url);
    if (!url) continue;
...
export function itemImage(item) {
  return (
    ...
    (item.enclosure && imageUrl(item.enclosure.url) &&
...
export function parseFeedDate(dateStr) {
