...
// V8 represents the result of slice()/trim()/substring() and regex captures
// as a view onto the parent string, not a copy. A 500-char description cut
// from a 50 KB content:encoded body, or an og:image URL captured from a 1 MB
// page, therefore pins the WHOLE parent in memory for as long as the small
// string lives — for the article store that is days, across thousands of
// items. Round-tripping through a Buffer yields a flat string that owns its
// bytes (utf16le is lossless for any JS string; V8 still stores it one-byte
// when it can). Strings under 13 chars are already copied by V8.
export function detach(text) {
  if (typeof text !== 'string' || text.length < 13) return text;
