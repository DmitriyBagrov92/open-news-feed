...
import { detach } from './normalize.js';
...
function extractImageUrl(html) {
  ...
  // The capture is a view onto the page HTML; cached as-is it would pin the
  // whole 1 MB prefix per entry (see normalize.detach).
  return detach(url);
}
...
async function fetchOgImage(url) {
  ...
  try {
    ...
    clearTimeout(timer);
    // Whatever path exits — a non-2xx, a refused redirect, an exception —
    // nothing of the request may linger: an unread body would otherwise
    // hold its socket and buffers until GC. No-op once the body is consumed.
    controller.abort();
  }
}
...
export async function enrichImages(articles, { budget = 30, concurrency = 4 } = {}) {
  ...
  try {
    ...
    log.warn('enrich failed', log.errorFields(err));
  }
