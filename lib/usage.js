...
function emptyWindow() {
  return {
    ...
    authors: new Set(),
    features: {},
    status: { ok: 0, client_error: 0, rate_limited: 0, not_implemented: 0, server_error: 0 },
...
export function usageMiddleware() {
  return (req, res, next) => {
    ...
    res.on('finish', () => {
      ...
      // 501 is the free tier's designed answer (summarize → premium-only,
      // translate → no-provider), not a fault — keep server_error meaningful.
      else if (s === 501) window.status.not_implemented += 1;
