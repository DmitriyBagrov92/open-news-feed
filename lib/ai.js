...
// Also hosts the per-IP rate limiter shared by the AI-ish routes
// (/api/article, /api/translate, /api/summarize).
...
export async function summarize(payload) {
  for (const provider of SUMMARIZE_PROVIDERS) {
    try {
      ...
      log.warn('summarize provider failed, falling back', log.errorFields(err));
    }
  }
...
export async function translateTexts(texts, target, source = 'en') {
  try {
    ...
    log.warn('translate provider failed, falling back', { provider: 'libretranslate', ...log.errorFields(err) });
  }
  try {
    const viaMyMemory = await myMemoryTranslate(texts, target, source);
    return { translations: viaMyMemory, provider: 'mymemory' };
  } catch (err) {
    log.warn('translate provider failed', { provider: 'mymemory', ...log.errorFields(err) });
  }
  throw new NoProviderError();
}

// ── rate limiter ─────────────────────────────────────────────────────────────
// Shared 30/min bucket for the AI-ish routes; other features create their
// own limiter instances via lib/ratelimit.js.

const aiLimiter = createLimiter({ limit: 30 });
