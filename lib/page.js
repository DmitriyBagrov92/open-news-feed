...
// English country names for the crawler list (the app localises its own).
const REGION_NAMES = new Intl.DisplayNames(['en'], { type: 'region' });
const REGION_EN = { '002': 'Africa', '019': 'Americas', 142: 'Asia', 150: 'Europe', '009': 'Oceania' };
export function countryNameEn(code) {
  if (!code) return '';
  if (REGION_EN[code]) return REGION_EN[code]; // UN M.49: not every ICU build names these
  try {
...
export function headlinesHtml() {
  ...
  const items = articles.map((a) => `
        <li>
          ...
          <span class="ssr-meta">${escapeHtml(a.source.name)}${
            countryNameEn(a.source.country) ? ` (${escapeHtml(countryNameEn(a.source.country))})` : ''} · <time datetime="${a.publishedAt}">${stamp(a.publishedAt)}</time></span>${
