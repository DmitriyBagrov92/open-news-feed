...
// Test fixture mode: the store is seeded from a JSON file and never touches
// the network (see seedFixture and lib/testmode.js).
...
// Called with stats() after every refresh cycle (IndexNow pings, …). A
// listener's failure is logged, never allowed to break the loop.
...
function toArticles(source, rawItems) {
  ...
  for (const raw of rawItems) {
    ...
    articles.push({
      ...
      // country is the registry's own string — shared, never copied per article
      source: { id: source.id, name: source.name, homepage: source.homepage, country: source.country ?? null },
...
export async function refreshAll() {
  if (FIXTURE) return; // fixture mode: the seed is the only source of truth
  ...
  try {
    ...
    for (const fn of refreshListeners) {
      Promise.resolve()
        .then(() => fn(stats()))
        .catch((err) => log.warn('refresh listener failed', log.errorFields(err)));
    }
...
export function startRefreshLoop() {
  if (FIXTURE) {
    seedFixture(FIXTURE).catch((err) => log.warn('fixture seed failed', log.errorFields(err)));
    return;
  }
...
// Seed the store from a fixture file: an array of
// { source, title, description, url, image, category?, ageMin }. Items
// carry `ageMin` (minutes before now) because absolute dates would age out
// of the 7-day window; everything else goes through toArticles()/rebuild()
// exactly as a real fetch would, so ids, dedupe and battle rules apply.
// `append` keeps what a source already holds (the "new stories" fixture).
export async function seedFixture(file, { append = false } = {}) {
  ...
  for (const item of raw) {
    ...
    bySource.get(item.source).push({
      ...
      publishedAt: new Date(now - (Number(item.ageMin) || 0) * 60_000),
    });
  }
  const known = new Set(sources.map((s) => s.id));
  const unknown = [...bySource.keys()].filter((id) => !known.has(id));
  if (unknown.length) log.warn('fixture names unknown sources', { sources: unknown.join(',') });
  let count = 0;
  for (const source of sources) {
    const items = bySource.get(source.id);
    if (!items) continue;
    const st = state.get(source.id);
    const fresh = toArticles(source, items);
    st.articles = append ? [...fresh, ...st.articles] : fresh;
    st.ok = true;
    st.lastError = null;
    st.lastFetchAt = now;
    count += fresh.length;
  ...
  log.info('fixture seeded', { file, articles: combined.length, items: count, append });
  return count;
}
