...
export async function refreshAll() {
  ...
  try {
    ...
    try {
      ...
      console.log(`[store] enriched ${enriched} images (${fetched} fetched)`);
    } catch (err) {
      console.warn(`[store] image enrichment failed: ${err.message || err}`);
    }
  } finally {
    refreshing = false;
  }
...
export function query({ category, q, sources: include, exclude, page, pageSize, lang, since, histogram } = {}) {
  ...
  const matched = combined.filter((a) => {
    if (BATTLE_ONLY_IDS.has(a.source.id)) return false; // battle view only
    if (!languages.has(a.language)) return false;
