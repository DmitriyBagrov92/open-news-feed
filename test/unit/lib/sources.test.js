...
// Sources with no single home country. Anything else must name one — a new
// source can never ship without telling the reader whose voice it is.
const INTERNATIONAL = new Set(['allafrica', 'gnews', 'newsdata', 'currents']);
...
test('every source declares where its publisher is based', () => {
  for (const s of all) {
    ...
    else assert.match(String(s.country), /^[A-Z]{2}$/, `${s.id}: country must be an ISO 3166-1 alpha-2 code`);
  }
...
test('every country in the registry has its flag vendored (npm run vendor:flags)', async () => {
  const codes = [...new Set(all.map((s) => s.country).filter(Boolean))];
...
test('a few well-known homes, so a bulk edit cannot silently scramble them', () => {
  ...
  const expected = { 'bbc-world': 'GB', aljazeera: 'QA', tass: 'RU', cgtn: 'CN', scmp: 'HK', 'npr-world': 'US', 'dw-world': 'DE', meduza: 'LV', euronews: 'EU', who: 'UN', 'fox-news': 'US' };
